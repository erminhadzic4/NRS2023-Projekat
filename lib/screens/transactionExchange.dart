...
class _TransactionExchangeState extends State<TransactionExchangePage> {
  ...
  Future getUserAccounts() async {
    ...
    final getAccounts = await http.get(
      Uri.parse('http://siprojekat.duckdns.org:5051/api/Account/user-accounts'),
  ...
  Future<void> fetchCurrencies() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        showDialog(
          ...
          builder: (context) => AlertDialog(
            title: Text('Error'),
            content: Text('Failed to fetch the available currencies: $_availableCurrencies'),
            actions: [
              TextButton(
                onPressed: () {
      ...
      showDialog(
        ...
        builder: (context) => AlertDialog(
          title: Text('Error'),
          content: Text('An error occurred while fetching the available currencies.'),
          actions: [
            TextButton(
              onPressed: () {
                Navigator.pop(context);
  ...
  Future<String?> getUserCurrency() async {
    ...
    if (accountsList.isNotEmpty) {
      final currency = accountsList[0]['currency']['name'];
  ...
  Future<void> getExchangeRate() async {
    ...
    try {
      final response = await http.get(
        Uri.parse("http://siprojekat.duckdns.org:5051/api/ExchangeRate?inputCurreny=$userCurrency&outputCurrency=$_selectedCurrency"),
        headers: <String, String>{
          'Content-Type': 'application/json; charset=UTF-8',
          'Authorization': 'Bearer $token'
        },
      ...
      if (response.statusCode == 200) {
        final responseData = json.decode(response.body);
        final exchangeRate = responseData['rate'] as double;
  ...
  Future<void> createExchangeTransaction() async {
    ...
    try {
      ...
      final response = await http.post(
          Uri.parse("http://siprojekat.duckdns.org:5051/api/Exchange/CreateExchangeTransaction"),
          headers: <String, String>{
            'Content-Type': 'application/json; charset=UTF-8',
            'Authorization': 'Bearer $token'
          },
          body: jsonEncode(<String, dynamic>{
            ...
            "recipientCurrency": _selectedCurrency,
          }));
      ...
      showDialog(
        ...
        builder: (context) => AlertDialog(
          title: Text('Error'),
          content: Text('An error occurred while creating the exchange transaction.'),
          actions: [
            TextButton(
              onPressed: () {
                Navigator.pop(context);
          ...
          child: Form(
            ...
            child: Column(
              ...
              children: [
                ...
                SizedBox(height: 16),
                Text('Exchange rate (Current currency to $_selectedCurrency): $_exchangeRate '),
                ElevatedButton(
                  onPressed: () {
